...
@title NVARCHAR(200), @author_id int, @category_id int, 
@published_year int, @description NVARCHAR(MAX), @quantity int
as
begin
...
CREATE or alter VIEW vw_authors AS
SELECT 
    id, 
    full_name, 
    bio, 
...
@full_name NVARCHAR(200), @bio NVARCHAR(MAX), @birthdate DATE
as
begin
INSERT INTO [dbo].[Authors] (full_name, bio, birthdate)
values (@full_name , @bio , @birthdate )
end

create proc upauthors
@id int,@full_name NVARCHAR(200), @bio NVARCHAR(MAX), @birthdate DATE
as
begin
UPDATE Authors 
SET full_name=@full_name, bio=@bio, birthdate=@birthdate 
WHERE id=@id
end

create  alter view vw_categories as
SELECT id, name, description 
FROM Categories with (nolock);

CREATE PROC insercategories
@name nvarchar(100), @description nvarchar(500)
as
begin
...
@id int,@name nvarchar(100), @description nvarchar(500)
as
begin
UPDATE Categories 
...
--
-- ------------------------------------------------------------
--  upbooks (removed duplicate old definition)
-- ------------------------------------------------------------
use BookDB
-- ==
-- ------------------------------------------------------------
...
-- ============================================================
--  AUTHORS — upauthors (cập nhật tác giả)
-- ============================================================
-- ------------------------------------------------------------
--  upauthors
--  Isolation: REPEATABLE READ
--  Lý do: Đảm bảo bản ghi tác giả không bị thay đổi bởi
--         session khác trong khi đang thực hiện UPDATE.
-- ------------------------------------------------------------
ALTER PROC upauthors
    @id         INT,
    @full_name  NVARCHAR(200),
    @bio        NVARCHAR(MAX),
    @birthdate  DATE
...
-- ============================================================
--  CATEGORIES — insercategories (thêm danh mục)
-- ============================================================
-- ------------------------------------------------------------
--  insercategories
--  Isolation: SERIALIZABLE
...
-- ============================================================
--  CATEGORIES — upcategories (cập nhật danh mục)
-- ============================================================
-- ------------------------------------------------------------
--  upcategories
--  Isolation: REPEATABLE READ
--  Lý do: Tương tự upbooks/upauthors — giữ lock trên bản ghi
--         đang UPDATE để tránh lost update.
-- ------------------------------------------------------------
ALTER PROC upcategories
    @id          INT,
    @name        NVARCHAR(100),
    @description NVARCHAR(500)
AS
...
ALTER PROC upbooks
    ...
    @quantity int,
    @row_ver BINARY(8),
    @new_row_ver BINARY(8) OUTPUT = NULL
AS
BEGIN
    SET NOCOUNT ON;
    SET XACT_ABORT ON;
    ...
    BEGIN TRY
        ...
        SELECT @new_row_ver = row_ver
        FROM Books
        WHERE id = @id;

        SELECT @new_row_ver AS new_row_ver;
...
ALTER TABLE Authors    ADD row_ver ROWVERSION NOT NULL;
ALTER TABLE Categories ADD row_ver ROWVERSION NOT NULL;
GO

ALTER PROC [dbo].[upauthors]
    @id         INT,
    @full_name  NVARCHAR(200),
    @bio        NVARCHAR(MAX),
    @birthdate  DATE,
