...
if (document.getElementById('logoutBtn')) {
  ...
  async function openBookModal(book = null) {
    ...
    document.getElementById('modalBookTitle').textContent = book ? 'Chỉnh Sửa Sách' : 'Thêm Sách';
    ...
    const [authors, categories] = await Promise.all([
      apiFetch('/authors'), apiFetch('/categories')
    ]);
    const aSelect = document.getElementById('bookAuthor');
    const cSelect = document.getElementById('bookCategory');
    aSelect.innerHTML = `<option value="">-- Chọn tác giả --</option>` +
      authors.map(a => `<option value="${a.id}" ${book?.author_id == a.id ? 'selected' : ''}>${a.full_name}</option>`).join('');
    cSelect.innerHTML = `<option value="">-- Chọn thể loại --</option>` +
      categories.map(c => `<option value="${c.id}" ${book?.category_id == c.id ? 'selected' : ''}>${c.name}</option>`).join('');
  ...
  document.getElementById('bookForm')?.addEventListener('submit', async (e) => {
    ...
    try {
      if (editingBookId) {
        await apiFetch(`/books/${editingBookId}`, { method: 'PUT', body: JSON.stringify(payload) });
  ...
  window.editAuthor = async (id) => {
    ...
    if (!a) return;
    editingAuthorId = id;
    document.getElementById('modalAuthorTitle').textContent = 'Chỉnh Sửa Tác Giả';
  ...
  document.getElementById('authorForm')?.addEventListener('submit', async (e) => {
    ...
    try {
      if (editingAuthorId) {
        await apiFetch(`/authors/${editingAuthorId}`, { method: 'PUT', body: JSON.stringify(payload) });
  ...
  window.editCategory = async (id) => {
    ...
    document.getElementById('modalCategoryTitle').textContent = 'Chỉnh Sửa Thể Loại';
  ...
  document.getElementById('categoryForm')?.addEventListener('submit', async (e) => {
    ...
    try {
      if (editingCategoryId) {
        await apiFetch(`/categories/${editingCategoryId}`, { method: 'PUT', body: JSON.stringify(payload) });
  ...
  document.getElementById('confirmDeleteBtn')?.addEventListener('click', async () => {
    ...
    const paths = { book: `/books/${id}`, author: `/authors/${id}`, category: `/categories/${id}` };
    try {
  ...
  apiFetch('/authors').then(d => document.getElementById('statAuthors').textContent = d.length).catch(() => {});
  apiFetch('/categories').then(d => document.getElementById('statCategories').textContent = d.length).catch(() => {});
}
