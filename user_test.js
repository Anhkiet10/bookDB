...
document
  ...
  .addEventListener("click", async () => {
    ...
    try {
      ...
      // const result = await apiFetch("/orders/safe", {
      const result = await apiFetch("/orders/deadlock-demo", {
        method: "POST",
        body: JSON.stringify({
      ...
      if (err.message === "price_changed") {
        // Reload lại giá mới từ server rồi hỏi lại user
        const fresh = await apiFetch(`/books/${currentBookForOrder.id}`);
        currentBookForOrder = fresh;
        document.getElementById("confirmPrice").textContent = formatPrice(
          fresh.price,
        );
        showToast(
          "Giá sách vừa thay đổi. Vui lòng xem lại và xác nhận.",
          "info",
        );
        btn.disabled = false;
        btn.innerHTML = `<i class="fas fa-check"></i> Xác nhận đặt`;
        return; // không đóng modal, để user tự bấm lại
      }
